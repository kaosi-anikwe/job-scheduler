...
class JobORM(TimestampMixin, Base):
    ...
    child_dependencies: Mapped[list["JobDependencyORM"]] = relationship(
        "JobDependencyORM",
        foreign_keys="JobDependencyORM.parent_job_id",
        cascade="all, delete-orphan",
        lazy="selectin",
    )

    # Parents that *this* job depends on (this job is the child)
    parent_dependencies: Mapped[list["JobDependencyORM"]] = relationship(
        "JobDependencyORM",
        foreign_keys="JobDependencyORM.child_job_id",
        cascade="all, delete-orphan",
        lazy="selectin",
    )
...
from shared.models.execution_log import ExecutionLogORM  # noqa: E402, F811
from shared.models.job_dependency import JobDependencyORM  # noqa: E402, F811
